...
!!! warning "图里的护栏位置是逻辑位置,不完全等于当前的物理部署位置"
    架构图把 Model Armor 护栏画在 `root_agent` 前后,这是数据流层面的**逻辑位置**——但当前的实现现状(见第 4、9 章)是这层检查以"外部脚本包一层"的方式实现的,还没有作为 ADK 的 `before_model_callback` / `after_model_callback` 跟着 agent 一起被部署到 Agent Runtime。换句话说,如果你直接调用第 6 章部署出来的远程 agent(不经过本地这层包装代码),护栏目前**不会生效**。这不是被隐藏的缺陷,而是第 9 章明确列出的生产化优化项之一——如实标注出来,是这份手册的写作原则之一。
...
如果你只想看某一块(比如只关心 RAG 怎么接),直接跳到对应章节即可,每章相对独立。第 8 章讲怎么把这套东西工程化(IaC/测试/CI),第 9 章是全局性的优化清单和参考资料汇总。

### 第 10、11 章:把"能跑"变成"能卖"

第 1～9 章回答的是同一类问题——**这东西怎么搭起来**。但把 agent 跑通只是 FDE 工作的上半场。真实的 discovery 会议上,demo 演示成功之后,接下来出场的是两类人,问的是前九章一个字都没覆盖的问题:

- **采购/业务方**问:一个月多少钱?量涨十倍成本怎么变?为什么选这个模型不选那个?
- **安全架构师**问:数据出境吗?能进我们的 VPC-SC 边界吗?用谁的密钥?高峰期吞吐有保证吗?

这两组问题答不上来,项目就停在 PoC,不会进生产。所以补了两章:

| 章节 | 解决什么 | 方法论上的差别 |
|---|---|---|
| **第 10 章 · 成本模型与选型** | 报价、TCO、模型/档位选型依据 | 不抄定价页,直接读 **Cloud Billing Catalog API**(账单系统自己的 SKU 目录) |
| **第 11 章 · 企业边界与数据驻留** | 数据驻留、VPC-SC、CMEK、身份、配额与 Provisioned Throughput | 不背文档,在**目标区域上跑探测脚本**,带实测数据进会议 |

这两章和前九章有一个共同点,也有一个明显不同。相同的是**可复现**:每个结论都附了取数脚本,你可以在自己的项目上重跑一遍。不同的是**保质期**:代码写对了就一直对,而价格和区域覆盖每个季度都在变。所以这两章的正文里大量出现"核查日期"和 ⬜ 空槽标记——⬜ 表示这一项在核查当天没能验证到,**留空而不是用估算值填充**。

!!! warning "这两章的数字会过期,方法不会"
    第 10、11 章里的所有价格和区域可用性数据,核查日期是 **2026-08-15**。不要直接把表格里的数字报给客户——**跑一遍章节里给的脚本,用当天的数据**。这两章真正想交付的不是那几张表,而是"客户会议前十分钟能重跑一遍"的取数流程。
